	...
	methods:
	{
		loadPasskeys()
		{
			...
			fetch(secure_passkeys_params.url, {
				...
				{
					action: "secure_passkeys_adminarea_get_profile_registered_passkeys_list",
			...
			.then((data) => {
				...
				{
					this.errorMessage = data.data.message || secure_passkeys_params.i18n.failed_fetch_passkeys;
				}
			})
			.catch((error) => {
		...
		deletePasskey(id)
		{
			var message = secure_passkeys_params?.i18n?.delete_message ?? "Are you sure you want to delete the passkey?";
			...
			jQuery.post(
				...
				function (data)
				{
					...
					else
					{
						...
						{
							that.errorMessage = secure_passkeys_params.i18n.failed_delete_passkey;
						}
					}
				},
				"JSON"
		...
		async postPasskeyData(params)
		{
			...
			.fail((jqXHR) => {
				const errorMessage = jqXHR.responseJSON?.error || this.i18n.failed_save_passkey;
				this.addingPasskey = false;
				this.creatingPasskey = false;
